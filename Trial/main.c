...
int main(int argc, char *argv[])
{
    ...
    int32_t channel = CAN_NETDEV;
    uint8_t op_mode = CANMODE_DEFAULT;
    unsigned int delay = 0;
    can_bitrate_t bitrate = { CANBDR_SOCKET };
    can_speed_t speed;
    can_status_t status;
    ...
    fprintf(stdout, "can_test: "__DATE__" "__TIME__" (Linux)\n");
    ...
    /* initialization */
    if((handle = can_init(channel, op_mode, &can_param)) < 0) {
        if(rc != CANERR_SOCKET)
